...
# Learned value net (optional). If value_net + weights.json are present, _eval
# returns the network's P(win); otherwise it uses the hand-crafted evaluation.
# The harness can force the baseline by setting _LEARNED_W = None.
try:
    import value_net as _VN
    _LEARNED_W = _VN.load_weights(os.path.join(os.path.dirname(__file__), "weights.json"))
except Exception:
    _VN = None
    _LEARNED_W = None

# Eval mode: _BLEND == 0 with weights -> pure learned value; _BLEND > 0 -> hand
# eval AUGMENTED by _BLEND*(P(win)-0.5); weights None -> pure hand-crafted eval.
_BLEND = 0.0
...
# =========================================================================== #
_USE_SEARCH = True
_STEP_BUDGET = 1500    # max search_step calls per decision (hard bound on work)
_ROLLOUT_STEPS = 40    # max steps to finish one turn rollout
...
_TIME_BUDGET = 1.5     # wall-clock seconds per decision (never near the limit)
...
def _eval(state, me):
    ...
    if res is not None and res >= 0:
        ...
        if res == (1 - me):
            return -1e6
        return 0.0  # draw
    if _LEARNED_W is not None and _BLEND <= 0.0:
        try:
            return _VN.value(_LEARNED_W, state, me)  # pure learned P(win) in [0,1]
        except Exception:
            pass  # any failure -> fall through to the hand-crafted evaluation
    opp = 1 - me
    mp = state.players[me]
    op = state.players[opp]
    score = 0.0
    ...
    if _LEARNED_W is not None and _BLEND > 0.0:
        try:
            score += _BLEND * (_VN.value(_LEARNED_W, state, me) - 0.5)  # augment
        except Exception:
            pass
    return score
...
def _rollout(search_id, obs, me, budget):
    ...
    while steps > 0:
        ...
        if st is not None and st.yourIndex != me:
            return _eval(st, me)  # control passed to opponent -> our turn is over
